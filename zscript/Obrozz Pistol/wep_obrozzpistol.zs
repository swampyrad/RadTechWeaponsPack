...
action void A_CheckRifleHand()
   ...
   //almost half the bulk due to cutting the stock and barrel
	...
	override void GunBounce(){
		...
		if(weaponstatus[OBROZZS_CHAMBER]>2&&!random(0,7))
...
//  no front sight, lol
	...
	user3:
		...
		goto ready;
	fire:
		#### A 1 A_JumpIf(invoker.weaponstatus[OBROZZS_CHAMBER]==2,"shoot");
		goto ready;
	shoot:
		#### A 1{
...
*/
	...
	altfire:
	    BROZ A 0 A_CheckRifleHand();
		#### A 1 A_WeaponBusy();
		#### B 2 A_JumpIf(invoker.weaponstatus[0]&OBROZZF_CUSTOMCHAMBER,1);
		#### B 1 A_JumpIf(invoker.weaponstatus[OBROZZS_CHAMBER]>2,"jamderp");
	...
	altholdclean:
		#### E 1 offset(2,36) A_ClearRefire();
		#### E 1 offset(3,38);
		#### E 1 offset(5,41) A_Log("Looking inside that chamber...",true);
		#### E 1 offset(8,44) A_StartSound("weapons/pocket",9);
		#### E 1 offset(7,50) A_MuzzleClimb(frandom(-0.2,0.2),0.2,frandom(-0.2,0.2),0.2,frandom(-0.2,0.2),0.2);
	...
	loadclip:
		#### A 0 A_JumpIf(invoker.weaponstatus[OBROZZS_MAG]>9,"reloaddone");
		#### A 3 offset(16,50){
